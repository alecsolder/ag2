...
# TASK_PROMPT = """The quesiton you will be asked is multiple choice. Figure out the multiple choice options and include a letter in your answer submission if relevant.."""
...
answerer = KnowledgeFunctionSwarmAgent(
    ...
    agent_system_message=ANSWERER_PROMPT % TASK_PROMPT,
    validator_system_message=ANSWERER_VALIDATOR_PROMPT % TASK_PROMPT,
...
#     open("./autogen/agentchat/contrib/toa/test_case.json", "r"))
...
def dynamic_chunking(text, max_tokens=100):
...
with open("./autogen/agentchat/contrib/toa/text_only.txt", "r") as file:
    text = file.read()

# Apply dynamic chunking
chunks = dynamic_chunking(text)
context = [["Whole Text", chunks]]
question = "Who is the most likely murderer Terrance or Nelson?"
...
"""
    ...
    had_some_memories = False
    for memory in beam_memories:
        if len(memory):
            had_some_memories = True

    if not had_some_memories:
        grade_required = max(0, grade_required - 25)
