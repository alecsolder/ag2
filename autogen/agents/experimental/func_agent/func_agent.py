...
{self.result_str}
"""
# Polyfill for markcoroutinefunction for Python < 3.12
def markcoroutinefunction(func):
    """Marks a function as a coroutine function."""
    if not asyncio.iscoroutinefunction(func):
        func.__code__ = func.__code__.replace(co_flags=func.__code__.co_flags | CO_COROUTINE)
    return func
...
# It uses the key {name}-FuncContext as the location it writes to
__all__ = ["FuncAgent"]


class FuncAgent(ConversableAgent):
    def __init__(
        self,
        name: str,
        ...
        **kwargs,
        ...
        self._context_variables_key = f'{self.name}-FuncContext'
    ...
    def _reply(self, messages: Optional[List[Dict]] = None,
        ...
        if sender_context is None:
            sender_context = {}
        reliable_function_context:FuncContext = self.run_func(task, messages, context_variables=sender_context)
    ...
    def run_func(self, task: str, messages: list[dict] = [], context_variables: dict = {}) -> FuncContext:
        ...
        reliable_function_context = FuncContext(task=task)
    ...
    async def a_run_func(self, task: str, messages: list[dict] = [], context_variables: dict = {}) -> FuncContext:
        # Needed because we rely on prompt for the task, could move it in to messages and not have to do this
        # Maybe could do something smarter by using UpdateSystemMessage
        self._update_prompts(task)
        self._invoked_messages = messages
    ...
    def decorator(tool_function):
        ...
        async def wrapper(*args, hypothesis="", context_variables={}, **kwargs) -> 'SwarmResult':
            try:
                ...
                context_state: FuncContext = _get_reliable_function_context(
...
def _get_reliable_function_context(context_variables, reliable_function_context_key):
    return FuncContext.model_validate_json(context_variables[reliable_function_context_key])
